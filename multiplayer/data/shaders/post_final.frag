...
// Only mod modifications under here (except draw_screen declaration): 2, 3, and 4 players splitscreen, else it does the vanilla lines
// Could be written shorter but at least should be understandable. Not generalized because for now 4 players splitscreen is already foolish.
	if (draw_screen.x == 2.0) {
		if (draw_screen.y == 1.0 && gl_FragCoord.y < window_size.y/2.0) {
        	gl_FragColor.rgb = color;
        	gl_FragColor.a = 1.0;
		} else if (draw_screen.y == 2.0 && gl_FragCoord.y >= window_size.y/2.0) {
			gl_FragColor.rgb = color;
    	    gl_FragColor.a = 1.0;
    	} else {
    	    gl_FragColor.a = 0.0;
    	}
	} else if (draw_screen.x == 3.0) {
		if (draw_screen.y == 1.0 && gl_FragCoord.y >= window_size.y/2.0) {
        	gl_FragColor.rgb = color;
        	gl_FragColor.a = 1.0;
		} else if (draw_screen.y == 2.0 && gl_FragCoord.y < window_size.y/2.0 && gl_FragCoord.x < window_size.x/2.0) {
			gl_FragColor.rgb = color;
    	    gl_FragColor.a = 1.0;
		} else if (draw_screen.y == 3.0 && gl_FragCoord.y < window_size.y/2.0 && gl_FragCoord.x >= window_size.x/2.0) {
			gl_FragColor.rgb = color;
    	    gl_FragColor.a = 1.0;
    	} else {
    	    gl_FragColor.a = 0.0;
    	}
	} else if (draw_screen.x == 4.0) {
		if (draw_screen.y == 1.0 && gl_FragCoord.y >= window_size.y/2.0 && gl_FragCoord.x < window_size.x/2.0) {
        	gl_FragColor.rgb = color;
        	gl_FragColor.a = 1.0;
		} else if (draw_screen.y == 2.0 && gl_FragCoord.y >= window_size.y/2.0 && gl_FragCoord.x >= window_size.x/2.0) {
			gl_FragColor.rgb = color;
			...
			gl_FragColor.rgb = color;
    	    gl_FragColor.a = 1.0;
		} else if (draw_screen.y == 4.0 && gl_FragCoord.y < window_size.y/2.0 && gl_FragCoord.x >= window_size.x/2.0) {
			gl_FragColor.rgb = color;
    	    gl_FragColor.a = 1.0;
    	} else {
    	    gl_FragColor.a = 0.0;
